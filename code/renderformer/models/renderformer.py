...
class RenderFormer(nn.Module, PyTorchModelHubMixin):
    def __init__(self, config: RenderFormerConfig):
        ...
        if self.config.use_sparse_attention:
            self.transformer = SparseTransformerEncoder(
                ...
                k_local=self.config.sparse_k_local,
                k_selected=self.config.sparse_k_selected,
                dropout=self.config.dropout,
                activation=self.config.activation,
                norm_type=self.config.norm_type,
                use_normal_mask=self.config.sparse_use_normal_mask,
    ...
    def forward(self, tri_vpos_list, texture_patch_list, valid_mask, vns, rays_o, rays_d, tri_vpos_view_tf, tf32_view_tf=False):
        ...
        # For sparse attention, pass normal information
        if self.config.use_sparse_attention:
            # Extract face normals from vertex normals (take mean of 3 vertex normals)
            # vns shape: [batch_size, max_num_tri, 3, 3] where each tri has 3 vertices with 3 coords
            tri_normals = vns.reshape(vns.size(0), vns.size(1), 3, 3).mean(dim=2)  # [batch_size, max_num_tri, 3]
            # Add register tokens normals (dummy normals pointing up)
            dummy_normals = torch.zeros(tri_normals.size(0), self.skip_token_num, 3, device=tri_normals.device)
            dummy_normals[..., 2] = 1.0  # Z-axis normal for dummy tokens
